...
小红书服务器通过浏览器自动化（Playwright）与平台交互，Twitter 服务器通过 CLI 工具与平台交互，这些方式均非官方授权。**使用本项目可能违反小红书和/或 X (Twitter) 的服务条款。** 一旦被检测到，你的账号可能面临临时或永久限制甚至封禁。此风险同时适用于你用于登录的账号以及你与之交互的账号。
...
你的认证凭据和会话令牌仅在本地处理（小红书使用 Playwright 浏览器上下文，Twitter 使用 CLI 子进程）。使用前请自行审查相关工具的安全实践。切勿将令牌或会话数据分享给他人。
